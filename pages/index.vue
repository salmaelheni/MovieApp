...
export default {
  name: "AllMovie",
  components: { LoadingComponent, HeroComponent,NavbarComponent },
  data() {
    return {
      movies: [],
      searchedMovies: [],
  ...
  methods: {
    async getMovies() {
      const result = await axios.get(`https://api.themoviedb.org/3/movie/now_playing?api_key=c3973a7fdda35b0058bef3411c380b1d`);
      result.data.results.forEach((movie) => {
        this.movies.push(movie);
      });
    },
    async searchMovies() {
      const result = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=c3973a7fdda35b0058bef3411c380b1d&language=en-US&page=1&query=${this.searchInput}`);
      result.data.results.forEach((movie) => {
